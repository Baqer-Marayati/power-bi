...
- For compact KPI cards, report-side `labelPrecision` and `labelDisplayUnits` are not always enough to change the rendered `bn / M` text. If Power BI keeps ignoring those settings, use dedicated text-returning KPI display measures and bind only the card visuals to them.
- Dedicated plain-number KPI alias measures were tested as a safer way to force two-decimal rendering on top cards, but in the live report they proved less reliable than the original measures. That helper layer has now been retired from `_Measures.tmdl`.
- Card caption text should not rely on helper measure names. The safer current pattern for the repeated top monetary cards is: bind directly to the real business measure, leave one visible category caption in the standard grey 9pt style, remove the extra visual title, and force non-compact display units at the card value layer.
- More specifically for these `cardVisual` objects: `categoryLabels.show = false` plus a separate title was not sufficient in live Desktop rendering. Screenshot validation showed the helper caption still rendered under the title, so treat that pattern as unsafe for the repeated top monetary cards.
