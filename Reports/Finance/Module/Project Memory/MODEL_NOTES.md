...
- **Card precision must always be explicit (2026-08-28 PM):** `cardVisual` does NOT inherit decimal
  ...
  Fleet rule: percent cards carry `labelPrecision = 2L`, bn money cards `3L`, explicitly. Never
  rely on "unset precision falls back to the model format" for cards.
- Money-card rule superseded same day (2026-08-28 PM, user decision): every money **total** KPI card binds the **raw** measure with card `labelDisplayUnits = 1000000000D` (Billions) and `labelPrecision = 3L` → "5.886bn د.ع." everywhere, all six reports. The `* Card Display` scaled helpers are no longer bound by any card — and were **deleted 2026-08-28 (same day, later pass)** after proof of non-use: zero report bindings, zero DAX references, zero bookmark/diagram references (19 measures per Financial model, 5 in Sales, plus 6 dangling Q&A linguistic entities in the Sales culture file). Do not reintroduce the pattern. Exempt cards render exact IQD (units `1D`, no precision): Avg Collection per Txn and the four cash cards (Paper's bank balance is ~66K IQD — bn would render 0.000bn). Rationale: one unit + one precision beats per-measure fixed scales, needs no compatibility-level upgrade, and 3 decimals = million resolution.
