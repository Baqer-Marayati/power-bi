...
- Model-level format strings completed for all numeric measures (2026-08-28): 67 additions each in the Canon/Paper Financial Fabric models (IQD triple `#,0\ "د.ع.‏";...` for amounts, `#,0` for counts, `0` for flag helpers, and a `formatStringDefinition` SWITCH on `Overview KPI Value` because it mixes IQD amounts with margin percents). Text/SVG/date helper measures intentionally stay unformatted.
