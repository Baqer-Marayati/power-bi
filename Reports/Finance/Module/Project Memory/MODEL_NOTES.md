...
- Money-card rule superseded same day (2026-08-28 PM, user decision): every money **total** KPI card binds the **raw** measure with card `labelDisplayUnits = 1000000000D` (Billions) and `labelPrecision = 3L` → "5.886bn د.ع." everywhere, all six reports. The `* Card Display` scaled helpers are no longer bound by any card (kept in the model for now; retire deliberately later — some may feed charts). Exempt cards render exact IQD (units `1D`, no precision): Avg Collection per Txn and the four cash cards (Paper's bank balance is ~66K IQD — bn would render 0.000bn). Rationale: one unit + one precision beats per-measure fixed scales, needs no compatibility-level upgrade, and 3 decimals = million resolution.
