...
- LC **closed** flag: `IsLcClosed = 1` when `OIPF.OpenForLaC = 'N'` **or** `OIPF.DocStatus = 'C'`. `LcDocumentStatus` uses the same OR rule (do not let `OpenForLaC = 'Y'` override a closed `DocStatus`). Page/report filters for “closed LC only” must use **`IsLcClosed`**, not the old label-only path. Validate counts against SAP after refresh.
