...
- **Line chart (*Add-on category % of landed by month*)** — The middle landed-cost trend should show add-on reporting categories over time, with one comparable Y-axis: **category add-on amount as % of total landed cost**. Exclude **Supplier base** from this visual, and keep the old **Top driver share %** out of the chart because it compresses the smaller operational category trends. Tooltips should carry add-on IQD, IQD/unit, total landed context, receipt qty, prior-month %, MoM pp change, share of add-ons, and category rank.
