...
- `[cursor]` `[powerbi]` The **`powerbi-remote` MCP interactive sign-in always fails** with `AADSTS50011`: the Azure app `ef8d7eae-96fd-4f5a-820d-e04030e70978` does not have `http://localhost:8787/callback` registered as a redirect URI. Do **not** retry `mcp_auth` — it will keep failing until an Azure admin adds that URI (App registrations → Authentication → Mobile and desktop applications).
