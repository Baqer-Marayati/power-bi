...
### 2026-08-18 — Power BI live access: skip MCP interactive auth, use service-principal REST

- `[cursor]` `[powerbi]` **FIXED 2026-08-18 (later same day):** `http://localhost:8787/callback` was added as a Mobile-and-desktop redirect URI on the Azure app `ef8d7eae-96fd-4f5a-820d-e04030e70978` ("Cursor Power BI Remote MCP"), and `mcp_auth` for `powerbi-remote` now succeeds — interactive MCP is the preferred path for live DAX/schema work. If `AADSTS50011` ever returns, check that redirect URI is still registered (App registrations → Authentication).
- `[repo]` `[powerbi]` The **working read-only path to live Power BI/Fabric** is the service principal (client-credentials) flow: creds in `Portfolio/scripts/powerbi-api-local.env` (gitignored), smoke test via `python3 Portfolio/scripts/test-powerbi-api-readonly.py powerbi-api-local.env`. The token works against `api.powerbi.com` REST — list workspaces/datasets and run **read-only DAX** via `POST .../datasets/<id>/executeQueries`.
- `[repo]` Known workspaces: **Paper Analytics** (`12bc0f31-cb6f-4e73-b3b6-64e63a8c8558`), **Canon Analytics** (`a0fbbdc7-e807-4b58-a39d-b45c2adc1477`), **Development Workspace** (`e25920dc-a13e-467a-b23e-ce3b95f0d595`). Live Paper Financial Report dataset in Paper Analytics: `96308a26-9381-4feb-886c-9c90afac6bf2`.
- `[repo]` **Direct HANA from this Mac fails without VPN**: `hana-vm-107` does not resolve (`hdbcli` is installed and fine; the DNS/VPN is the blocker). Prefer the Power BI REST path for live-model questions; use HANA only from the Windows VM or on VPN.
- `[general]` `[powerbi]` **ChatGPT custom MCP connector cannot connect to the Fabric Power BI MCP (as of 2026-08-18):** ChatGPT requires the OAuth server metadata to advertise PKCE (`code_challenge_methods_supported` with `S256`), but Microsoft Entra's discovery document omits that field (verified directly; the RFC 8414 endpoint 404s). Fails at connector creation regardless of client ID/scopes/redirect URI setup. Workarounds: use Cursor (proven), or host a patched static metadata document and point ChatGPT's "Authorization server base" at it (hack, not recommended). Recheck periodically — a one-line Microsoft metadata fix would unblock it.
